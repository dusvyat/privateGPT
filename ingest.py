...
def load_single_document(file_path: str) -> Document:
    ...
    return loader.load() #removed [0] from here for csv, csv is a row per document
...
def load_csv(source_dir: str) -> List[Document]:
    # Loads all documents from source documents directory
    csv_files = glob.glob(os.path.join(source_dir, "**/*.csv"), recursive=True)
    return [load_single_document(file_path) for file_path in csv_files]
...
def main():
    ...
    documents = load_csv(source_directory)
